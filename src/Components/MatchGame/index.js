...
import './index.css'
import Corousels from '../Corousels/index'
...
class MatchGame extends Component {
  ...
  onCheckingSelection = id => {
    const {activeImgObject} = this.state
    const activeImgId = activeImgObject.id
    if (activeImgId === id) {
      const newActiveImgObj =
        imagesList[Math.ceil(Math.random() * imagesList.length)]
      this.setState(prevState => ({
        score: prevState.score + 1,
        activeImgObject: newActiveImgObj,
      }))
    } else {
      this.setState({isGameActive: false})
    }
  ...
  getFilteredThumbNails = () => {
    ...
    const filteredThumNails = imagesList.filter(
      eachObject => eachObject.category === activeTabId,
    )
    return filteredThumNails
  }

  getGamePlayBody = () => {
    const {activeTabId, activeImgObject} = this.state
    const filteredThumbNails = this.getFilteredThumbNails()
    return (
      <>
        <ul className="carousel-img-list">
          <Corousels
            activeImgObject={activeImgObject}
            key={activeImgObject.id}
          />
        </ul>
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <Tabs
              tabDetails={eachTab}
              key={eachTab.tabId}
              isSelected={eachTab.tabId === activeTabId}
        ...
        <ul className="thumbnails-container">
          {filteredThumbNails.map(eachObject => (
            <ThumbNails
              details={eachObject}
              key={eachObject.id}
              onCheckingSelection={this.onCheckingSelection}
            />
          ))}
        </ul>
      </>
  ...
  getGameOverBody = () => {
    ...
    return (
      <div className="game-result-body">
        ...
        <h1 className="game-score-text">{score}</h1>
        <button
          type="button"
  ...
  render() {
    const {isGameActive, score, timeRemaining} = this.state
    ...
    return (
      <div className="app-container">
        ...
        <div className="game-body">
          ...
          {!isGameActive && (
            <div className="game-over-body">{this.getGameOverBody()}</div>
          )}
        </div>
      </div>
    )
